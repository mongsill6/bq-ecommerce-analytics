...
log_info "광고 ROAS 리포트 ($START_DATE ~ $END_DATE)"
...
log_warn "적자 캠페인 (ROAS < 1): ${DEFICIT_COUNT}개"
...
log_warn "중단 검토 키워드: ${STOP_KEYWORDS}개"

if [ "$STOP_KEYWORDS" -gt 0 ]; then
  log_info "중단 검토 키워드 목록:"
...
log_info "총 캠페인: ${TOTAL_CAMPAIGNS}개"
log_info "총 광고비: ${TOTAL_SPEND}원"
log_info "총 매출: ${TOTAL_REVENUE}원"
log_info "종합 ROAS: ${OVERALL_ROAS}"
log_info "적자 캠페인: ${DEFICIT_COUNT}개"
log_info "중단 검토 키워드: ${STOP_KEYWORDS}개"
log_info "시트 탭: 캠페인ROAS, 적자캠페인, 광고효율추이, 키워드성과, 주간추이"
log_success "광고 ROAS 리포트 업데이트 완료: https://docs.google.com/spreadsheets/d/$SPREADSHEET_ID"
